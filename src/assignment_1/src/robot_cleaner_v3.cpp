...
int main(int argc, char **argv)
{
	...
	cout<<"desired absolute orientation: ";
	cin>>desired_angle_degrees;
	setDesiredOrientation (degrees2radians(desired_angle_degrees));
...
void setDesiredOrientation (double desired_angle_radians){
	double psi = atan2((2*(turtlebot_pose.x*turtlebot_pose.d+turtlebot_pose.y*turtlebot_pose.z))/(turtlebot_pose.x^2+turtlebot_pose.y^2-turtlebot_pose.z^2-turtlebot_pose.w^2))
	double relative_angle_radians = desired_angle_radians - psi;
	bool clockwise = ((relative_angle_radians<0)?true:false);
	//cout<<desired_angle_radians <<","<<psi<<","<<relative_angle_radians<<","<<clockwise<<endl;
	rotate (degrees2radians(10), abs(relative_angle_radians), clockwise);

}

void poseCallback(const nav_msgs::Odometry::ConstPtr & pose_message){
	turtlebot_pose.x=pose_message->x;
	turtlebot_pose.y=pose_message->y;
	turtlebot_pose.z=pose_message->z;
	turtlebot_pose.w=pose_message->w;
}
...
void gridClean(){
	...
	for(int i=1; i<3; i=i+1){
		move(1, 9, true);
		rotate(degrees2radians(30), degrees2radians(85), true);
		loop.sleep();
		move(1, 2, true);
		rotate(degrees2radians(30), degrees2radians(85), true);
		loop.sleep();
		move(1, 9, true);
		rotate(degrees2radians(30), degrees2radians(85), false);
		loop.sleep();
		move(1, 2, true);
		rotate(degrees2radians(30), degrees2radians(85), false);
		loop.sleep();
