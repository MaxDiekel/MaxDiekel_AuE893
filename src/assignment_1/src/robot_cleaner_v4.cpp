...
void move(double speed, double distance, bool isForward){
	...
	double t1 = speed/accel;
	double t2 = distance/speed;
	double t3 = speed/accel + distance/speed;
	double current_distance = 0.0;
	int delta_t = t1/10;
	double delta_speed = accel*delta_t;
	ros::Rate loop_rate(delta_t);
	for(double sp = 0; sp <= speed; sp = sp + delta_speed){
		if (isForward)
			vel_msg.linear.x =abs(sp);
		else
			vel_msg.linear.x =-abs(sp);
	...
	velocity_publisher.publish(vel_msg);
	sleep(t2-t1);
	for(double sp = speed; sp >= 0; sp = sp - delta_speed){
		if (isForward)
			vel_msg.linear.x =abs(sp);
		else
			vel_msg.linear.x =-abs(sp);
...
void gridClean(){
	...
	for(int i=1; i<3; i=i+1){
		move(.5, 9, true);
		rotate(degrees2radians(15), degrees2radians(80), true);
		loop.sleep();
		move(.5, 2, true);
		rotate(degrees2radians(15), degrees2radians(80), true);
		loop.sleep();
		move(.5, 9, true);
		rotate(degrees2radians(15), degrees2radians(80), false);
		loop.sleep();
		move(.5, 2, true);
		rotate(degrees2radians(15), degrees2radians(80), false);
		loop.sleep();
